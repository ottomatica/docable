...
class Stepper {
    ...
    async buildConnector(setupObj, cwd, docDir) {
        ...
        for (let target of setupObj.targets) {
            connectors.push({ name: target.slug || target.name, cwd: target.cwd, connector: await Infra.select(target, target.cwd || cwd, docDir) });
        }
    ...
    async runSteps(cells, $, connectors, cwd, stepIndex, vars) {
        ...
        for (const cell of cells) {
            ...
            const op = new Operators(cellTargetConnector.connector, cellTargetConnector.cwd);
