...
class Setup {
    ...
    async extractSetup(doc)
    {
        const $ = cheerio.load(await fs.promises.readFile(doc, { encoding: 'utf-8' }));

        let setupString;

        let comments = $.root().contents().filter(function () { return this.type == 'comment'; });
        if( comments && comments.length > 0 ) {
            setupString = comments.get(0).data;
        } else { // provide default setup string
     ...
     */
    async setup(doc, html, setupObj = undefined) {
        ...
        // if inline
            ...
            setupString = await this.extractSetup(doc);
        }

        setupObj = setupObj || yaml.safeLoad(setupString).setup;
        
        // if any setup config specified`
        if(setupObj) {
            const docDir = path.resolve(path.dirname(doc));
            const provider = Object.keys(setupObj)[0];
            let cwd = provider === 'local' ? docDir : '.';
