        ...
        for( let step of doc.steps )
        {
            if( step instanceof Object )
            {
                ...
                switch( key )
                {
                    case "selectAsFile":
                    {
                        let searchText, file, permission;
                        ...
                        steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText), await sl.selectAsFile( sl.asContainsNext($, searchText), file, user, persistent, target, permission )));
                        break;
                    }
                    case "selectAndServe":
                    {
