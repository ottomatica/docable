...
class StepsReader {
    ...
    async read(stepFile)
    {
        ...
        let {conn,cwd,docPath,targets,clean} = await setup.setup(document, stepFile);

        return {docs: await this.documents(document, docPath), conn: conn, cwd: cwd, targets: targets,clean:clean};
    }
