...
class StepsReader {
    ...
    async annotationSteps(metadata)
    {
        ...
        for( let code of Object.keys(metadata) )
        {
            ...
            let insertafter = metadata[code].insertafter;
            let insertbefore = metadata[code].insertbefore;
            ...
            if( file )
            {
               steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAsFile( code, file, user, persistent, target, {insertafter, insertbefore} )));
            }
            else if( serve )
            {
        ...
        for( let step of doc.steps )
        {
            if( step instanceof Object )
            {
                ...
                for (const k of Object.keys(step)) {
                    ...
                    else if (k === 'append') insertafter = step[k]
                    else key = k
                }
                ...
                switch( key )
                {
                    case "selectAsFile":
                    {
                        ...
                        steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText), await sl.selectAsFile( sl.asContainsNext($, searchText), file, user, persistent, target, {insertafter, insertbefore} )));
                        break;
                    }
                    case "selectAndServe":
                    {
                    ...
                    case "selectAndRun":
                    {
                        if( step[key].expectblock && step[key].select )
                        {
                            ...
                            steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText).add(sl.asContainsNextElement($, step[key].expectblock)), await sl.selectAndExpect( sl.asContainsNext($, searchText), sl.asContainsNext($, step[key].expectblock), user, persistent, target )) );
                            break;
                        }
                        else
                        {
                            let searchText = step[key];
                            steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText), await sl.selectAndRun( sl.asContainsNext($, searchText), user, persistent, target )) );
                            break;
                        }
                    }
                    default:
