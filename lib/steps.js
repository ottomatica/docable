                ...
                switch( key )
                {
                    ...
                    case "selectAndRun":
                    {
                        if( step[key].expectblock && step[key].select )
                        {
                            ...
                            steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText).add(sl.asContainsNextElement($, step[key].expectblock)), await sl.selectAndExpect( sl.asContainsNext($, searchText), sl.asContainsNext($, step[key].expectblock))) );
                            break;
                        }
                        else
                        {
