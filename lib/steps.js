...
class Steps {
    ...
    async annotationSteps(metadata)
    {
        ...
        for( let code of Object.keys(metadata) )
        {
            ...
            if( file )
            {
               steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAsFile( code, file )));
            }
            else if( serve )
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAndServe( code )));
            }
            else if( expect )
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAndExpect( code )));
            }
            else if( run )
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAndRun( code )) );
            }
        }
        return steps;
    }
