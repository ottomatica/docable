...
class Steps {
    ...
    async read(stepFile)
    {
        ...
        return {docs: await this.documents(document, cwd), conn: conn, cwd: cwd};
    }
    ...
    async annotationSteps(metadata)
    {
        ...
        for( let code of Object.keys(metadata) )
        {
            ...
            if( file )
            {
               steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAsFile( code, file, user )));
            }
            else if( serve )
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAndServe( code, user )));
            }
            else if( expect )
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAndExpect( code, user )));
            }
            else if( run )
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.selectAndRun( code, user )) );
            }
        }
        return steps;
    }
