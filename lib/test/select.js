...
class Select {
    ...
    async selectAsFile(content, destination, user, persistent, target, permission)
    {
        ...
        {
            try {
                let output = await this.op.file( content, destination, user, persistent, target, permission);
    ...
    async generateExpectScript(input, user, target) {
        ...
        await this.op.file(`${input.cmd} > /tmp/${cmd_md5}.stdout 2> /tmp/${cmd_md5}.stderr\necho $? >> /tmp/${cmd_md5}.stdout`, cmdScript, user, false, target, 'x');
        ...
        await this.op.run('command -v expect || sudo apt-get install -y expect', user, undefined, target);
