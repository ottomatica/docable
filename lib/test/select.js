...
class Select {
    ...
    async selectAsFile(content, destination, user, target, permission)
    {
        return new Promise(async (resolve, reject) => 
        {
            let output = {};
            try {
                await this.op.run(`sudo mkdir -p ${path.dirname(destination)}`, user, undefined, target);
                output = await this.op.file( content, destination, user, target, permission);
    ...
    async generateExpectScript(input, user, target) {
        const template = await fs.promises.readFile(path.resolve(__dirname, './input.mustache'), { encoding: 'utf8' });
        const cmd_md5 = md5(input.cmd);
        const cmdScript = `/tmp/${cmd_md5}.sh`;
        await this.op.file(`#!/bin/bash\n\n${input.cmd.trimRight()} 2> /tmp/${cmd_md5}.stderr | tee /tmp/${cmd_md5}.stdout \necho "\${PIPESTATUS[0]}" >> /tmp/${cmd_md5}.stdout`, cmdScript, user, target, 'x');
        input.cmd = cmdScript.toString();
        const expectScript = mustache.render(template, input);
        const expectScriptPath = `/tmp/${cmd_md5}.expect`;
        await this.op.file(expectScript, expectScriptPath, user, target, 'x');

        // ensuring expect is installed
        // TODO: is there a better way to check this?
        await this.op.run('command -v expect || (sudo apt-get update && sudo apt-get install -y expect && sudo rm -r /var/lib/apt/lists/*)', user, undefined, target);
        await this.op.run(expectScriptPath, user, undefined, target);

        return `( cat /tmp/${cmd_md5}.stdout && cat /tmp/${cmd_md5}.stderr >&2 )`;
    }

    async selectAndExpect(content, external, user, persistent, target, opts = { prompts: undefined })
    {
        if(opts.prompts) {
        ...
        return new Promise(async(resolve, reject) => 
        {
            ...
            // updating exitCode
            if (opts.prompts) {
                output.stdout = output.stdout.trimRight();
            ...
            if( this.splitLines(expect).every( line => output.stdout.includes(line.trim())) )
            {
    ...
    async selectAndRun(content, user, persistent, target, opts = { prompts: undefined })
    {
        // opts.prompts = [
