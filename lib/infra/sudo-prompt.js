      ...
      function(error, uuid) {
        ...
        Node.fs.mkdir(instance.path,
          function(error) {
            ...
            let stdoutLines = 0;
            let stderrLines = 0;
            Node.fs.watch( Node.path.dirname(instance.pathStdout), {persistent: false}, async (event, filename) => {
                if ( event== 'change' && filename=='stdout' ) {
                    let data =(await Node.fs.promises.readFile(instance.pathStdout)).toString();
                ...
                if ( event== 'change' && filename=='stderr' ) {
                    let data =(await Node.fs.promises.readFile(instance.pathStderr)).toString();
                    let lines = data.split(/\r?\n/);
