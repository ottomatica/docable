/* Patched version of sudo-prompt to support streaming */
...
let stdout_events = new Node.events.EventEmitter();
let stderr_events = new Node.events.EventEmitter();
                ...
                function(error, stdout, stderr) {
                  ...
                  MacCommand(instance,
                    function(error) {
                      ...
                      // Watch stdout/stderr for changes and emit data events.
                      watchFileProgress( Node.path.join(instance.path, 'Contents', 'MacOS') );

                      MacOpen(instance,
                        function(error, stdout, stderr) {
                          // Stop listening.
                          watchFileProgressStop( Node.path.join(instance.path, 'Contents', 'MacOS') );
                          if (error) return end(error, stdout, stderr);
...
function MacCommand(instance, end) {
  ...
  let cwd = instance.options.cwd || Node.process.cwd();
...
function watchFileProgressStop(baseDirectory)
{
  Node.fs.unwatchFile(Node.path.join(baseDirectory,'stdout'));
  Node.fs.unwatchFile(Node.path.join(baseDirectory,'stderr'));
}


function watchFileProgress(baseDirectory)
{
  let stdoutLines = 0;
  let stderrLines = 0;

  console.log( baseDirectory );
  Node.fs.watchFile(Node.path.join(baseDirectory,'stdout'), async (curr, prev) => {
      // console.log(`the current size is: ${curr.size}`);
      // console.log(`the previous size was: ${prev.size}`);

      if( curr.size > 0 )
      {
          let data =(await Node.fs.promises.readFile(Node.path.join(baseDirectory,'stdout'))).toString();
          let lines = data.split(/\r?\n/);
          // console.log( lines.length, stdoutLines )
          // console.log( data );

          if( lines.length > stdoutLines)
          {
              stdout_events.emit('data', lines.slice(stdoutLines).join("\n").trim() );
              stdoutLines = lines.length-1;
          }    
      }
  });      

  Node.fs.watchFile(Node.path.join(baseDirectory,'stderr'), async (curr, prev) => {
      ...
      if( curr.size > 0 ) 
      {
          let data =(await Node.fs.promises.readFile(Node.path.join(baseDirectory,'stderr'))).toString();
          let lines = data.split(/\r?\n/);
          // console.log( data );
          if( lines.length > stderrLines)
          {
              stderr_events.emit('data', lines.slice(stderrLines).join("\n").trim() );
              stderrLines = lines.length-1;
          }
      }
...
function progress(baseDirectory)
{
  let stdoutLines = 0;
  let stderrLines = 0;
  Node.fs.watch( baseDirectory, {persistent: false, recursive: true}, async (event, filename) => {
      console.log( filename, event );
      if ( event== 'change' && filename=='stdout' ) {
          let data =(await Node.fs.promises.readFile(Node.path.join(baseDirectory,'stdout'))).toString();
          let lines = data.split(/\r?\n/);
          // console.log( lines.length, stdoutLines )
          if( lines.length > stdoutLines)
      ...
      if ( event== 'change' && filename=='stderr' ) {
          let data =(await Node.fs.promises.readFile(Node.path.join(baseDirectory,'stderr'))).toString();
          let lines = data.split(/\r?\n/);
          if( lines.length > stderrLines)
          {
      ...
      // when directory is deleted in windows
      // console.log(err);
  });    
}
    ...
    function(error, uuid) {
      ...
      Node.fs.mkdir(instance.path,
        function(error) {
          ...
          // Watch stdout/stderr for changes and emit data events.
          watchFileProgress( instance.path );
...
function WindowsWriteCommandScript(instance, end) {
  var cwd = instance.options.cwd || Node.process.cwd();
  if (/"/.test(cwd)) {
    // We expect double quotes to be reserved on Windows.
    // Even so, we test for this and abort if they are present.
...
module.exports.stdout_events = stdout_events;
module.exports.stderr_events = stderr_events;
