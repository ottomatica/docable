...
class Operators {
    ...
    async file(content, location, user, persistent, target, opts = {insertafter:false, insertbefore: false})
    {
        ...
        if(!opts.insertafter && !opts.insertbefore) 
            cmd = `${this.sudoCMD(user, conn)} tee ${location.trim()} << 'END'\n${content}\nEND\n`;
        else if(opts.insertafter) 
            cmd = `sed -i '/${insertafter}/a ${content}' ${location.trim()}`;
        else if(opts.insertbefore)
            cmd = `${this.sudoCMD(user, conn)} tee ${location.trim()} << 'END'\n${content}\nEND\n`;
        
        let output;
        if (persistent) output = await conn.execPersistent(cmd, persistent);
        else output = await conn.exec(cmd);
