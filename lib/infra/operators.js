...
class Operators {
    ...
    {
        return await this._run(cmd, onProgress, target);
    }

    // Simple command
    async run(cmd, target)
    {
        return await this._run(cmd, null, target)
    }
    ...
    _privileged(cmd, onProgress) {
        return new Promise(function(resolve, reject)
        {
            ...
            sudo.exec(cmd, {name: 'Docable privileged command'}, function (err, stdout, stderr)
            {
                if( err && err.message )
                { 
    ...
    async _run(cell, onProgress, target)
    {
        ...
        { 
            output = await _privileged(cmd, onProgress);
        }
        else if( stream ) {
