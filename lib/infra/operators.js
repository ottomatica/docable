...
class Operators {
    ...
    async file(content, location, user, persistent, target)
    {
        ...
        if(location.includes('/'))
            ...
            if (persistent) output = await conn.execPersistent(`${this.sudoCMD(user, conn)} tee ${path.join(this.cwd, location.trim())} << 'END'\n${content}\nEND\n`, persistent);
            else output = await conn.exec(`${this.sudoCMD(user, conn)} tee ${path.join(this.cwd, location.trim())} << 'END'\n${content}\nEND\n`);
        }
    ...
    async run(cmd, user, persistent, target)
    {
        ...
        // if (persistent) output = await conn.execPersistent(`cd ${this.cwd} && ${this.sudoCMD(user, conn)} ` + cmd, persistent);
        // else output = await conn.exec(`cd ${this.cwd} && ${this.sudoCMD(user, conn)} ` + cmd);
