...
class Operators {
    ...
    async file(content, location, user, target, permission) {
        ...
        const tempPath = path.join(os.tmpdir(), randomName);

        try {
            await fs.promises.writeFile(tempPath, content);
            await conn.scp(tempPath, `/tmp/${randomName}`);
            output = await conn.exec(`${this.sudoCMD(user, conn)} mv /tmp/${randomName} ${location}`);
