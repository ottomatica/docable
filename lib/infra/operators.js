...
class Operators {
    ...
    async file(content, location, user, target, permission, mode) {
        ...
        try {
            ...
            if( location.startsWith('~/') != 0 && !path.isAbsolute(location) )
            {
            ...
            {
                await conn.exec(`${this.sudoCMD(user, conn)} mkdir -p ${path.dirname(location)}`);
            }
...
`);
    ...
    async _run(cell, onProgress, target)
    {
        ...
        let {user, persistent, privileged, shell, stream, spawn, tty} = cell;
        ...
        if( cell.path )
        {
            let cwd = cell.path;
            // relative paths should respect cwd.
            if( cell.path.startsWith('~/') != 0 && !path.isAbsolute(cell.path) )
            {
        ...
        else if ( cell.interactive && conn.type == "local" )
        {
            ...
            {
                let scriptPath = await this._cmdAsBashScript(cmd);
